...
//
// Instagram shows part of the composer UI over/around the preview and can make
// edge-to-edge artwork look cropped even when the file is technically 4:5.
// Keep the exported canvas at the canonical 1080x1350, but render the artwork
// itself at 92% inside it. This gives every important element a predictable
// safe area without changing the actual image ratio expected by the feed.
...
const MAX_ITEMS = 10;
const INSTAGRAM_SAFE_SCALE = 0.92;
...
export const FeedCapsuleArt = forwardRef<View, FeedCapsuleArtProps>(function FeedCapsuleArt(
  ...
  // `canvas` controls the real exported file dimensions. `u` controls only the
  // artwork inside the canvas, keeping it safely away from Instagram's edges.
  const canvas = (n: number) => n * scale;
  const u = (n: number) => n * scale * INSTAGRAM_SAFE_SCALE;
  ...
  return (
    ...
    >
      <View style={{ width: u(1080), height: u(1350), backgroundColor: C.bg, padding: u(68), overflow: 'hidden' }}>
