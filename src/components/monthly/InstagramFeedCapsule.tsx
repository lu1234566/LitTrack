...
export const InstagramFeedCapsule: React.FC<Props> = ({ data, id = 'instagram-feed-capsule', exportSafe = false }) => {
            ...
            <div className="relative group px-6">
              ...
              <div className="relative bg-neutral-900/60 p-10 rounded-[3rem] border border-amber-500/10 flex gap-8 items-center">
                <div className="w-24 h-36 bg-neutral-800 rounded-xl overflow-hidden shadow-2xl shrink-0 relative">
                  {getCoverSrc(data.bestBook.id, data.bestBook.coverUrl || data.bestBook.ilustracaoUrl) ? (
                    <img
                      src={getCoverSrc(data.bestBook.id, data.bestBook.coverUrl || data.bestBook.ilustracaoUrl)}
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center p-4 text-center bg-gradient-to-br from-neutral-800 to-neutral-900">
                      ...
                      <span className="text-[10px] text-neutral-600 font-bold uppercase line-clamp-3">{data.bestBook.titulo}</span>
                    </div>
                  )}
                </div>
                <div className="min-w-0">
                  ...
                  <p className="text-lg text-amber-400 font-bold mt-2">Nota {Number(data.bestBook.notaGeral || 0).toFixed(1)} • {data.bestBook.pageCount || 0} págs</p>
                </div>
              </div>
            </div>
          )}
              ...
              <div key={book.id} className="flex items-center gap-8 bg-neutral-900/20 p-8 rounded-[2.5rem] border border-white/5 hover:bg-neutral-900/30 transition-all">
                ...
                <div className="w-20 h-28 bg-neutral-900 rounded-lg overflow-hidden shrink-0 border border-white/10 shadow-lg relative">
                  {getCoverSrc(book.id, book.coverUrl || book.ilustracaoUrl) ? (
                    ...
                    <div className="w-full h-full flex flex-col items-center justify-center p-3 text-center bg-gradient-to-br from-neutral-800 to-neutral-900">
                      ...
                      <span className="text-[8px] text-neutral-600 font-bold uppercase line-clamp-3">{book.titulo}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="min-w-0 flex-grow">
                  <div className="flex justify-between items-start gap-4 mb-2">
                    <h3 className="text-3xl font-bold text-neutral-50 truncate max-w-[70%]">{book.titulo}</h3>
                    <div className="flex items-center gap-2 shrink-0">
                      <Star size={18} className="text-amber-500 fill-amber-500" />
                      <span className="text-2xl font-black text-amber-400">{Number(book.notaGeral || 0).toFixed(1)}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-6 font-mono text-xl text-neutral-600">
                    <span className="italic font-serif text-neutral-500 truncate max-w-[60%]">{book.autor}</span>
