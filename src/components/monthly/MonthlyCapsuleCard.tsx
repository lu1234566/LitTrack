...
import { BookOpen, Clock, Star, Award, Layers, Ghost } from 'lucide-react';
    ...
    >
      ...
      <div className="relative z-10 space-y-2">
        <div className="flex justify-between items-start">
          ...
          <span className="text-[10px] text-neutral-500 tabular-nums">{new Date().toLocaleDateString('pt-BR')}</span>
        </div>
        <h1 className="text-4xl font-serif leading-tight">
          Cápsula de <span className="text-amber-100 italic">{monthName}</span>
        </h1>
        {userName && <p className="text-xs text-neutral-400 font-sans tracking-wide">Jornada de {userName} • {year}</p>}
      </div>

      <div className="relative z-10 py-4 border-y border-neutral-800/50">
        <p className="text-sm font-serif italic text-neutral-300 leading-relaxed text-center">"{literaryCopy}"</p>
      </div>

      <div className="relative z-10 grid grid-cols-2 gap-4">
        <StatItem icon={<BookOpen className="w-4 h-4 text-amber-500/70" />} label="Livros Lidos" value={totalBooks} />
        <StatItem icon={<Layers className="w-4 h-4 text-neutral-500" />} label="Páginas do Mês" value={totalPages} />
        <StatItem icon={<Clock className="w-4 h-4 text-neutral-500" />} label="Tempo de Foco" value={`${hours}h ${minutes}m`} />
        <StatItem icon={<Star className="w-4 h-4 text-amber-500/70" />} label="Média do Mês" value={averageRating.toFixed(1)} />
      </div>
      ...
      <div className="relative z-10 space-y-4 flex-grow pr-1">
        <h3 className="text-[10px] uppercase tracking-[0.2em] text-neutral-500 font-bold flex items-center gap-2 sticky top-0 bg-neutral-950/80 backdrop-blur-sm py-1">
          ...
          Livros e Avaliações de {monthName} ({booksCompleted.length})
        </h3>

        {booksCompleted.length > 0 ? (
          <div className="space-y-3">
            {booksCompleted.map((book) => (
              <div key={book.id} className="flex gap-4 items-center group bg-neutral-900/30 p-2 rounded-lg border border-neutral-800/10 hover:border-neutral-800/50 transition-all">
                ...
                <div className="min-w-0 flex-grow">
                  <div className="flex justify-between items-start gap-2">
                    ...
                    <span className="text-[11px] font-bold text-amber-400 shrink-0">{(book.notaGeral || 0).toFixed(1)}</span>
                  </div>
                  <div className="flex justify-between items-end mt-1 gap-2">
                    <p className="text-[9px] text-neutral-500 truncate italic font-serif">{book.autor}</p>
                    {book.pageCount ? <span className="text-[8px] text-neutral-600 font-mono">{book.pageCount} págs</span> : null}
                  </div>
                </div>
              </div>
            ))}
        ...
        {achievements.length > 0 && (
          <div className="flex flex-wrap gap-2 justify-center">
            {achievements.map((ach, idx) => (
              <span key={idx} className="px-2 py-1 bg-amber-500/10 border border-amber-500/20 text-[8px] text-amber-400 rounded-full flex items-center gap-1">
...
const StatItem = ({ icon, label, value }: { icon: React.ReactNode; label: string; value: string | number }) => (
  <div className="bg-neutral-900/50 border border-neutral-800/50 p-3 rounded-lg space-y-1 group hover:border-neutral-700 transition-colors">
    ...
    <div className="text-xl font-serif text-amber-50 transition-transform group-hover:scale-105 origin-left duration-300">{value}</div>
  </div>
);
