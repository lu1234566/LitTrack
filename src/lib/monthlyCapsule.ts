...
const isTimestampInTargetMonth = (value: unknown, monthStart: Date, year: number) => {
  const numeric = safeParseNumber(value as number);
  if (!numeric) return false;
  const date = new Date(numeric);
  return isSameMonth(date, monthStart) && date.getFullYear() === year;
};
...
export const getMonthlyStats = (
  ...
  if (averageRating >= 9 && ratedBooks.length >= 2) achievements.push('Mês de Ouro');
...
export const getInstagramCapsuleData = (
  ...
  const sortedBooks = [...stats.booksCompleted].sort((a, b) => {
    const aRating = safeParseNumber(a.notaGeral);
    const bRating = safeParseNumber(b.notaGeral);
    if (bRating !== aRating) {
