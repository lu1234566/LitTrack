...
export default function LiteraryProfileScreen() {
  ...
  const archetype = resolveArchetype(stats.favoriteGenre, stats.averageRating, stats.finishedBooks, favoriteQuotes);
  ...
  return (
    <Screen>
      ...
      <Card>
        ...
        <Text style={styles.quote}>“Quanto mais livros e citações forem registrados, mais preciso fica o seu retrato de leitor.”</Text>
...
function resolveArchetype(genre: string, averageRating: number, finishedBooks: number, favoriteQuotes: number) {
  const lowerGenre = genre.toLowerCase();
  if (lowerGenre.includes('fantasia')) return 'Leitor de mundos';
  if (favoriteQuotes >= 3) return 'Guardião de trechos';
  if (finishedBooks >= 8) return 'Leitor disciplinado';
