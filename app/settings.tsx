...
export default function SettingsScreen() {
  ...
  async function save() {
    await updatePreferences({ readerName, favoriteFormat, reminderText, syncUserId, yearlyGoal: Number(yearlyGoal) || 0, dailyPageGoal: Number(dailyPageGoal) || 0 });
    setSyncMessage('Preferências salvas localmente.');
  }

  async function pushAll() {
    const nextPreferences = { ...preferences, readerName, favoriteFormat, reminderText, syncUserId, yearlyGoal: Number(yearlyGoal) || 0, dailyPageGoal: Number(dailyPageGoal) || 0 };
    ...
    <Screen>
      ...
      <Card>
        ...
        <View style={[styles.layoutGrid, mobile && styles.stack]}>
          <LayoutOption title="Automático" text="Adapta-se ao tamanho da tela" active />
          <LayoutOption title="Desktop" text="Força o layout de computador" />
          <LayoutOption title="Mobile" text="Força o layout de celular" />
        </View>
      </Card>

      <Card>
        <View style={styles.inlineHeader}><Text style={styles.cardTitle}>♧ Lembretes Literários</Text><View style={styles.toggle}><Text style={styles.toggleText}>Ativado</Text></View></View>
        <Text style={styles.kicker}>FREQUÊNCIA E HORÁRIO</Text>
        <View style={styles.chips}><Text style={styles.chipActive}>Diário</Text><Text style={styles.chip}>Dias úteis</Text><Text style={styles.chip}>Semanal</Text></View>
...
function LayoutOption({ title, text, active = false }: { title: string; text: string; active?: boolean }) {
  return <View style={[styles.layoutOption, active && styles.layoutOptionActive]}><Text style={styles.layoutIcon}>▱</Text><Text style={styles.layoutTitle}>{title}</Text><Text style={styles.layoutText}>{text}</Text></View>;
}
